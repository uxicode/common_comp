 ...
 * @param paging
 */
function getTotalPageCount(paging: { total: number, numOfPage: number }): number {
 ...
 * @param paging
 */
function getPageNum(paging: { totalPageCount: number, pageSize: number, curPageNum: number }): number[] {
  const { start, end }=getPageRange( paging );
 ...
 * @param paging
 */
function getPageRange(paging: { totalPageCount: number, pageSize: number, curPageNum: number }): { start: number, end: number } {
  //total - 전체 개수
  //numOfPage - 페이지 당 개수
...
export {getTotalPageCount, getPageNum};
